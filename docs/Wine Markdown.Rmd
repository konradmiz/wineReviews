...
* The majority of wines in the dataset are from the United States (50k) and most of those are from California (33.5k). In total there are `r length(unique(wine$country))` countries represented
...
* Pinot Noir is the most commonly rated wine, though there are `r length(unique(wine$variety))` varieties 


* There are some data quality issues (e.g., California is listed as both a province and a region_1). This isn't relevant to my analysis but would need to be fixed if I were going in another direction
...
This returns a vector of the most popular (most-reviewed) wines with length `r wine_vars_num`. These wines are: 
`r paste(top_vars, collapse = ", ")`
